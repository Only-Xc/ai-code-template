...
def make_auth_service(db: Session) -> AuthService:
    return AuthService(db, settings=settings, send_email=lambda **_: None)
...
def test_create_user(db: Session) -> None:
    ...
    user = make_auth_service(db).create_user(user_create=user_in)
...
def test_authenticate_user(db: Session) -> None:
    ...
    password = random_lower_string()
    user_in = UserCreate(email=email, password=password)
    auth_service = make_auth_service(db)
    user = auth_service.create_user(user_create=user_in)
    authenticated_user = auth_service.authenticate(email=email, password=password)
...
def test_not_authenticate_user(db: Session) -> None:
    email = random_email()
    password = random_lower_string()
    user = make_auth_service(db).authenticate(email=email, password=password)
    assert user is None
...
def test_update_user(db: Session) -> None:
    ...
    if user.id is not None:
        auth_service.update_user(db_user=user, user_in=user_in_update)
    user_2 = db.get(User, user.id)
    assert user_2
    assert user.email == user_2.email
...
def test_authenticate_user_with_bcrypt_upgrades_to_argon2(db: Session) -> None:
    ...
    authenticated_user = make_auth_service(db).authenticate(
